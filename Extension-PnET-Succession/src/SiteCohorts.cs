...
namespace Landis.Extension.Succession.BiomassPnET
{
    public class SiteCohorts : ISiteCohorts, Landis.Library.BiomassCohorts.ISiteCohorts, Landis.Library.AgeOnlyCohorts.ISiteCohorts
    {
        ...
        //Heat Load Index to modify probability of establishment in PnET extenstion
        //Added by A.Keyser Sept 2020
        ...
        public bool Grow(List<IEcoregionPnETVariables> data)
        {
            ...
            // set initial values for SpringPPT and yearCounter for use in adjusting SW establishment probability
            // AKeyser Oct 2020
            double SpringPPT = 0;
            int yearCounter = 0;            //Clear pressurehead site values
            ...
            for (int m = 0; m < data.Count(); m++ )
            {
                ...
                if (data[m].Month == (int)Constants.Months.December)
                {
                    ...
                    //added by chang 06/25/21; calculate spring precip
                    for (int i = (4 + yearCounter * 12); i < (7 + yearCounter * 12); i++)
                    {
                        SpringPPT += data[i].Prec;
                    }
                    yearCounter += 1;
                    //
            ...
            if (PlugIn.ModelCore.CurrentTime > 0)
            {
                //load variables of groundSlope and uphillSlopeAzimuth from the DFFS extension - chang - 07/01/21
                groundSlope = PlugIn.ModelCore.GetSiteVar<ushort>("Fire.groundSlope");
                uphillSlopeAzimuth = PlugIn.ModelCore.GetSiteVar<ushort>("Fire.UphillSlopeAzimuth");
                //Heat Load Index for adjusting survival for SW US forests
                //akeyser Oct2020
                double HLI = GetHeatLoadIndex(Site);

                // AKeyser Oct 2020
                double muSpringPPT = SpringPPT / yearCounter;
                foreach (ISpeciesPNET spc in PlugIn.SpeciesPnET.AllSpecies)
                {
                    bool estab = false;
                    float pest = 0;
                    if (monthlyCount[spc] > 0)
                    {
                        ...
                        float pestOG = cumulativeFwater[spc] * cumulativeFrad[spc] * spc.MaxPest;
                        // calcuate the probability of survival based on topography and Spring precipitation for US SW forests                     
                        float pSurvSW = (float)(-3.6997366 + (-1.987008 * HLI) + (0.1538613 * groundSlope[Site]) + (0.04982965 * muSpringPPT));
                        //multiply the original PnET pest by the US SW survival probability
                        pest = pestOG * pSurvSW;
                        //pest = pestOG;
                        //Console.WriteLine(muSpringPPT);
                        //Console.WriteLine(yearCounter);
                    }
...
//                        ToRemove.AddRange(cohorts[spc].Where(o => o.Age == speciescohort[c].Age));
        ...
        // Heat load index calculation added for use in adjusting seedling survival for forests in US Southwest
        // AKeyser Oct 2020
        public static double GetHeatLoadIndex(ActiveSite site)
        {
            ///define variable and set to default value of 0.0
            double hlii = 0.0;

            //pull aspect of out uphillazimuth
            ushort aspect = uphillSlopeAzimuth[site];

            //first have to reverse aspect back to traditional aspect from uphill azimuth
            if (uphillSlopeAzimuth[site] >= 180 && uphillSlopeAzimuth[site] <= 360)   //slope is North facing    
            {
                aspect = (ushort)(uphillSlopeAzimuth[site] - 180);
            }
            else
            {
                aspect = (ushort)(uphillSlopeAzimuth[site] + 180);
            ...
            // fold the aspect such that NE=0 and SW=1
            ushort foldAspect = (ushort)(180 - Math.Abs(aspect - (ushort)225));

            //calculate simple heat load index McCune and Keon (2002) J.Veg.Sci.
            hlii = (1 - Math.Cos(foldAspect - 45)) / 2;
